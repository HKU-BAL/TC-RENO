...
class bigGenePred(object):
    ...
    def from_string(self, string_bed):
        ...
        self.chrom=string_l[0][0:] # revsere the +chr operation
...
#        self.geneType=string_l[19]
    ...
    def to_bedstr(self, gene_start=None):
        ...
        for exon in self.exon:
            start, end = exon
            str_one = "\t".join([self.chrom, str(start), str(end), self.name])#,str(self.score),self.strand])
        ...
        for intron in self.intron:
            start, end = intron
            str_one_intron = "\t".join([self.chrom, str(start), str(end), self.name])#,str(self.score),self.strand])
...
#        junction_lf= junction_l if self.strand=="+" else (junction_l)[::-1]
