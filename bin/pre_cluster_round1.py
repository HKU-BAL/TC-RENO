...
import scipy
import operator
from pre_round1  import summary,pandas_summary1,wrapper_bedtools_intersect3,wrapper_bedtools_intersect2, bigglist_to_bedfile, pandas_summary, add_subread_bigg, get_readall_bigg
from utils import del_files


def flow_cluster(bigg_nano, bigg_gff, key1, cutoff="auto", intronweight=2.3):
...
def prefilter_smallexon(bigg_list,bigg_list_gff, key1,cutoff=50):
    ...
    if len(bigg_list_strand)==0:
        return None
    nano_exon, nano_intron=bigglist_to_bedfile(bigg_list_strand,key1)
    gff_exon, gff_intron=bigglist_to_bedfile(bigg_list_gff,key1)
...
def filter_D(D, bigg_list, add_miss=False):
    cutoff=0.025
