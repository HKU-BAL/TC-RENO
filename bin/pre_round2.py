...
def bigglist_to_bedfile(bigg_list,key1,prefix=None, dir=sys.argv[5]):
    surfix=str(randint(100, 999))

    bigg0=bigg_list[0]
    if prefix is None:
        prefix=key1 +bigg0.name
    if dir is None:
        dir=set_tmp()
  ...
  #  for line in f.readlines():
   #     print line
    ...
    cmd="bedtools intersect -wo  -a {bedfile1}_s -b {bedfile2}_s>{out}".format(
        bedfile1=bedfile1, bedfile2=bedfile1, out=outfile)
   # print cmd, bedfile1, bedfile2
    _=myexe(cmd)
   ...
   # else:
    with open(bed8file) as f:
            for line in f:
                ...
                line = line.split('\t')
#                print line[3], line[9]
                D_intersect[pos_dic[line[3]], pos_dic[line[7]]] += int(line[8])
